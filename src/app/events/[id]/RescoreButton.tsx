...
export default function RescoreButton({ eventId }: Props) {
  ...
  async function handleRescore() {
    ...
    setLoading(true);
    try {
      const res = await fetch("/api/events/" + eventId + "/rescore", { method: "POST" });
  ...
  return (
    <div className="space-y-2">
      ...
      {done && !error && <p className="text-xs text-emerald-400">Rescored — page updated.</p>}
